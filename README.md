# 〈第2.1世代〉ころボ / Korobo (2-1 gen)
<!-- ![〈第2.1世代〉ころボ / KOROBO (2-1 gen)](/Document/image_korobo-2-1.jpg) -->
<img src="/Document/Picture/image_korobo-2-1.jpg" width="75%">

## 概要 / Overview
準備中．．．
## ファイル構成 / Directory Layout
- [**Board**](Board): 基板発注に必要なガーバーデータ
- [**Code**](Code): ソースコード，ライブラリ
- [**Document**](Document): 購入リスト，画像など
- [**Model**](Model): STEPファイルおよびSTLファイル
## 組み立て / How to Set Up
### 0. 部品調達
#### 0.1 購入 
[Document](Document)の "[Parts-List_KOROBO_2-1.pdf](/Document/Parts-List_KOROBO_2-1.pdf)" に記載されている各種材料を，添付されているURLの通販サイトや実店舗などにて，調達してください．
#### 0.2 印刷
> [!TIP]
> KORokoro.rOBOt projectの[BOOTH](https://korokoro-robot.booth.pm/)にて，<ins>3Dプリンタ製部品</ins>を購入された方は，"[**1. 基板**](#1-基板)" へ進んでください．
<details open>
<summary>手順</summary>

a
</details>

***
### 1. 基板
#### 1.0 準備
#### 1.1 発注 or 作成
> [!TIP]
> KORokoro.rOBOt projectの[BOOTH](https://korokoro-robot.booth.pm/)にて，<ins>メインボード</ins>を購入された方は，"[**1.2 半田付け**](#12-半田付け)" へ進んでください．
<details>
<summary>業者に発注される方</summary>

a
</details>
<details>
<summary>ユニバーサル基板から作成される方</summary>

b
</details>

#### 1.2 半田付け
---
### 2. 配線
> [!TIP]
> KORokoro.rOBOt projectの[BOOTH](https://korokoro-robot.booth.pm/)にて，<ins>配線類</ins>を購入された方は，"[**3. 駆動ユニット**](#3-駆動ユニット)" へ進んでください．
<details open>
<summary>手順</summary>

#### 2.0 準備
#### 2.1 ピンヘッダー
#### 2.2 JST-XH
#### 2.3 JST-PH
#### 2.4 PicoBlade
</details>

---
### 3. 駆動ユニット
#### 3.0 準備
#### 3.1 TAMIYA製ギアボックス
#### 3.2 タイヤ
#### 3.3 ギアボックスカバー
---
### 4. はめ込み
#### 4.0 準備
#### 4.1
